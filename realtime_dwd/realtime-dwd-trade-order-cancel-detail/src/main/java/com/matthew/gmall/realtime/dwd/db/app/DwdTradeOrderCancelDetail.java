...
public class DwdTradeOrderCancelDetail extends BaseSQLAPP {
    ...
    public void handle(StreamTableEnvironment tableEnv, StreamExecutionEnvironment env) {
        ...
        //3.过滤取消订单的数据，op=u status变为1003
        Table orderCancelInfo = tableEnv.sqlQuery(
                "select \n" +
                        "\tafter['id'] as id,\n" +
                        ...
                        "and op = 'u' and before['order_status'] <> '1003' and after['order_status'] = '1003'"
        );
        ...
        //把数据写入到kafka对应主题中
        resultTable.insertInto(Constant.TOPIC_DWD_TRADE_ORDER_CANCEL).execute();
    }
}
